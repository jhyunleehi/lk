...
이제 사용자 모드 프로세서 레지스터가 저장되었으므로 alltraps는 커널 C 코드를 실행하도록 프로세서 설정을 마칠 수 있습니다.  CPU는 인터럽트 핸들러에  들어가기 전에 선택기 %cs 및 %ss를 설정합니다. alltraps는 %ds 및 %es를 설정합니다. %fs 및 %gs가 SEG_KCPU CPU당 데이터 세그먼트를 가리키도록 설정합니다.
